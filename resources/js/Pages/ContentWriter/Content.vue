...
const postContent = () => {
    ...
    axios.post(route('postContent'), editForm.data()).then(r => {
        location.href = r.data.authorization_url
    }).catch(e => {
...
onMounted(() => {
  props.config.events.initialized()
})
</script>
...
<section class="max-w-[1400px] w-full px-4 mx-auto">
    <froala id="edit" :tag="'textarea'" :config="config" v-model:value="editForm.content"></froala>
    <InputError :message="editForm.errors.contenu" />
     <div class="mt-6">
